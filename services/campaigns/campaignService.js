  ...
  buildSearchFilters (campaignData, brandData) {
    ...
    if (brandData.ai_generated_overview) {
      ...
      if (aiOverview.collaboration_fit?.ideal_creators) {
        ...
        } else if (idealCreators.includes('food')) { filters.niche = 'food_cooking' } else if (idealCreators.includes('travel')) {
    ...
    try {
      ...
      const campaignQuery = `
        INSERT INTO campaigns (
          ...
          content_guidelines, hashtags, mention_requirements, approval_required, brand_owner_id
        ) VALUES (
      ...
      const campaignValues = [
        ...
        campaignData.brand_owner_id = userId
      ]
  ...
  async getCampaignById (campaignId, userId = null, userRole = null) {
    try {
      ...
      if (result.rows.length === 0) {
        console.warn(`No active campaign found with ID: ${campaignId}`)
        return null
      }
      ...
      const parseJsonField = (fieldValue) => {
        if (fieldValue && typeof fieldValue === 'string') {
          try {
            ...
            console.error(
              `Failed to parse JSON string for field: ${fieldValue}`,
              e
            )
            // Depending on requirements, you might return null, the original string, or throw
            return null // Or return fieldValue;
          }
        }
        return fieldValue // Return as is if already an object, null, or not a string
      }

      // Parse JSON fields safely
      campaign.target_audience = parseJsonField(campaign.target_audience)
      campaign.requirements = parseJsonField(campaign.requirements)

      // Specifically for ai_recommended_influencers, apply the safe parsing
      if (campaign.ai_recommended_influencers) {
        campaign.ai_recommended_influencers = parseJsonField(
      ...
      if (!isOwner && !isAdmin) {
        ...
        return {
          ...
          brand_owner_id: campaign.brand_owner_id,
          status: campaign.status,
          description: campaign.description,
          brand_name: campaign.brand_name,
          brand_verification: campaign.brand_verification,
          created_at: campaign.created_at
          // Ensure ai_recommended_influencers is not exposed here if it's sensitive
        }
      }
      return campaign
    } catch (error) {
